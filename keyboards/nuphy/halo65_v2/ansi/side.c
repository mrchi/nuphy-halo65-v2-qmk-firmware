...
const uint8_t side_led_index_tab[50] =
    {
        SIDE_INDEX + 10,     
        SIDE_INDEX + 11,
        SIDE_INDEX + 12,
...
uint8_t	key_pwm_tab[50] = {0x00};
uint8_t power_play_index = 0;
uint8_t f_power_show = 1;
uint8_t is_side_rgb_on(uint8_t index)
{
    if((((index >= 0)&&(index <= 10))||((index >= 43)&&(index <= 44)))&&(f_side_flag&0x01)) return true;
    else if((((index >= 11)&&(index <= 17))||((index >= 23)&&(index <= 34))||((index >= 36)&&(index <= 42)))&&(f_side_flag&0x02)) return true;
    else if(((index >= 45)&&(index <= 49))&&(f_side_flag&0x04)) return true;
    else if(((index >= 18)&&(index <= 22))&&(f_side_flag&0x08)) return true;
    else if(((index = 35))&&(f_side_flag&0x10)) return true;
    else return false;
}
...
static void side_wave_mode_show(void)
{
    ...
    for (int i = 0; i <= side_line - 5; i++) {
        if (side_rgb) {
            ...
            light_point_playing(1, 4, FLOW_COLOUR_TAB_LEN, &play_index);
    ...
    for (int i = 0; i <= 45; i++) {
        if(i == 45)
        {
            if(f_side_flag == 0x1f){
                ...
                g_temp = g_temp * 0.3;
                b_temp = b_temp * 0.3;            
                for(;i<50;i++) rgb_matrix_set_color(side_led_index_tab[i], r_temp, g_temp, b_temp);
                return;  
            } else {
                for(;i<50;i++) rgb_matrix_set_color(side_led_index_tab[i], 0, 0, 0);
                return;  
            }            
        }
        if(is_side_rgb_on(i)) rgb_matrix_set_color(side_led_index_tab[i], r_temp, g_temp, b_temp);
...
void rf_led_show(void)
{
    ...
    if (dev_info.link_mode == LINK_RF_24) 
    {
        ...
        r_temp = colour_lib_1[2][0];
        g_temp = colour_lib_1[2][1];
        b_temp = colour_lib_1[2][2];
